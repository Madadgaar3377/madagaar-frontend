...
export default function LoanApply() {
    ...
    (async () => {
      ...
      try {
        const res = await fetch(
          `${LOAN_POST_API}/${encodeURIComponent(planId)}`
        );
  ...
  function validateStep(currentStep = step) {
    // Basic per-step validation. Extend as needed.
    if (currentStep === 0) {
    ...
    if (currentStep === 1) {
      if (!form.netSalary || !String(form.netSalary).trim())
        return "Please enter your net salary or income.";
    }
    if (currentStep === 2) {
      if (!form.city || !form.city.trim()) return "Please enter your city.";
    }
    // no hard validation for references step - optional
    return null;
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          body?.message || body?.error || "Failed to submit form"
        );
      ...
      // navigate away after brief pause so user sees success
      ...
      <div className="flex items-center gap-2 mt-4">
        {steps.map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            ...
            <div
              className={`text-xs ${
                i === step ? "text-gray-800" : "text-gray-500"
              }`}
            >
              {s}
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            ...
            >
              ...
              <option value="Corporate / Commercial Loan">
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600">
              Monthly Income From Other Source
            </label>
            <input
              value={form.monthlyIncomeFromOtherSrc}
              onChange={(e) =>
                updateField("monthlyIncomeFromOtherSrc", e.target.value)
              }
              className="mt-1 w-full px-3 py-2 border rounded-md"
            />
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            ...
            <input
              ...
              onChange={(e) => updateField("materialStatus", e.target.value)}
              className="mt-1 w-full px-3 py-2 border rounded-md"
            />
          </div>
          <div>
      ...
      <div className="space-y-4 bg-gray-50 p-3 rounded-md border">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3">
          <input
            ...
            onChange={(e) =>
              updateField("commonForm.reference", e.target.value)
            }
            ...
            {form.commonForm.typeOfInquiry.map((row) => (
              <div key={row.id} className="flex gap-2 items-start">
                <input
                  ...
                  value={row.inquiry}
                  onChange={(e) =>
                    updateInquiryRow(row.id, "inquiry", e.target.value)
                  }
                ...
                <input
                  ...
                  value={row.comment}
                  onChange={(e) =>
                    updateInquiryRow(row.id, "comment", e.target.value)
                  }
                  className="flex-1 px-3 py-2 border rounded"
                />
                <button
                  type="button"
                  onClick={() => removeInquiryRow(row.id)}
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600">
            Additional Notes / Previous Loan History
          </label>
          <textarea
      ...
      <div className="space-y-4">
        <div className="p-4 bg-gray-50 rounded border">
          ...
          <div className="mt-3 text-sm text-gray-700 space-y-2">
            ...
            <div>
              <strong>Net Salary:</strong> {form.netSalary || "—"}
            </div>
            <div>
              <strong>Loan Plan:</strong>{" "}
              {form.typeOfLoan || (plan && plan.title) || "—"}
            </div>
            <div>
              <strong>Previous Loan History:</strong>{" "}
              {form.previousLoanHistory || "—"}
            </div>
          </div>
        </div>
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div className="flex-1">
                    <div className="text-sm font-semibold text-gray-800">
                      {plan.title || plan.planBy}
                    </div>
                    <div className="text-xs text-gray-500 mt-1">
                      Provider: {plan.planBy || plan.user?.businessName || "—"}{" "}
                      • Amount: {plan.loanAmount || "—"}
                    </div>
                    <div className="mt-2 text-sm text-gray-600">
                      {(plan.description || "").slice(0, 180)}
                    </div>
                  </div>
                </div>
              ) : (
              ...
              <div className="flex gap-2">
                ...
                {step < steps.length - 1 && (
                  <button
                    ...
                    className="px-4 py-2 rounded bg-[rgb(183,36,42)] text-white"
                  >
                    Next
                  </button>
                )}
