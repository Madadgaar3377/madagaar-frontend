    ...
    async (opts = {}) => {
      try {
        ...
        if ((opts.search ?? search).trim())
          params.search = (opts.search ?? search).trim();
        if (opts.status !== undefined ? opts.status : status)
          params.status = opts.status ?? status;
        if (opts.isVerified !== undefined ? opts.isVerified : isVerified)
          params.isVerified = opts.isVerified ?? isVerified;
        ...
        if (res.data && res.data.success) {
          ...
          if (res.data.pagination) {
            ...
            setPagination({
              ...
              totalPages: 1,
              totalItems: Array.isArray(res.data.data)
          ...
          <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
            ...
            {loading && (
              <div className="p-8 text-center text-gray-500 text-sm">
                Loading installment applications...
              </div>
            )}
                        ...
                        <td className="px-4 py-3 align-top hidden md:table-cell">
                          <div className="flex flex-col gap-1">
                            <span className="text-xs font-medium text-gray-800">
                              {app.productName ||
                                app.installmentPlan?.productName}
                            </span>
                            <span className="text-[11px] text-gray-500">
                              Plan:{" "}
                              {app.installmentPlan?.installmentPlanId || "N/A"}{" "}
                              • Posted By: {app.postedBy}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3 align-top hidden md:table-cell">
                          <div className="flex flex-col gap-1 text-xs">
                            <span className="text-emerald-600 font-semibold">
                              Total: Rs.{" "}
                              {app.totalPrice?.toLocaleString("en-PK") || "N/A"}
                            </span>
                            <span className="text-gray-600">
                              Down: Rs.{" "}
                              {app.downPayment?.toLocaleString("en-PK") ||
                                "N/A"}
                            </span>
                            <span className="text-gray-500 text-[11px]">
                              Monthly: Rs.{" "}
                              {app.installmentAmount?.toLocaleString("en-PK") ||
                                "N/A"}{" "}
